...
func NewSnapshotter(root string, opts ...Opt) (snapshots.Snapshotter, error) {
	...
	if err := os.Mkdir(filepath.Join(root, "writable", "snapshots"), 0700); err != nil && !os.IsExist(err) {
		return nil, err
	}
...
func (o *snapshotter) Stat(ctx context.Context, key string) (info snapshots.Info, err error) {
	...
	if o.upperdirLabel {
		...
		if info.Kind == snapshots.KindActive {
			info.Labels[upperdirKey] = o.upperPathWritable(id)
		} else {
...
func (o *snapshotter) Usage(ctx context.Context, key string) (_ snapshots.Usage, err error) {
	...
	if info.Kind == snapshots.KindActive {
		...
		if isExtracting(key) {
			...
			upperPath = o.upperPathWritable(id)
		}
...
func (o *snapshotter) Mounts(ctx context.Context, key string) (_ []mount.Mount, err error) {
	...
	return o.mounts(s, key, info), nil
}
...
func (o *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	return o.ms.WithTransaction(ctx, true, func(ctx context.Context) error {
		...
		usage, err := fs.DiskUsage(ctx, upperPath)
		if err != nil {
			return err
		}
...
func (o *snapshotter) getCleanupDirectories(ctx context.Context) ([]string, error) {
	...
	snapshotRdDir := filepath.Join(o.root, "snapshots")
	...
	snapshotWrDir := filepath.Join(o.root, "writable", "snapshots")
...
func (o *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) (_ []mount.Mount, err error) {
	...
	if err := o.ms.WithTransaction(ctx, true, func(ctx context.Context) (err error) {
		...
		if isExtracting(key) {
			...
			snapshotDir = filepath.Join(o.root, "writable", "snapshots")
		}
...
func (o *snapshotter) mounts(s storage.Snapshot, key string, info snapshots.Info) []mount.Mount {
	var options []string
	...
	if len(s.ParentIDs) == 0 {
		...
		src := ""
		if isExtracting(key) {
			src = o.upperPath(s.ID)
		} else {
			src = o.upperPathWritable(s.ID)
		}
		return []mount.Mount{
			{
				Source: src,
	...
	if s.Kind == snapshots.KindActive {
		if isExtracting(key) {
			...
			options = append(options,
				fmt.Sprintf("workdir=%s", o.workPathWritable(s.ID)),
				fmt.Sprintf("upperdir=%s", o.upperPathWritable(s.ID)),
			)
		}
...
func (o snapshotter) upperPathWritable(id string) string {
	return filepath.Join(o.root, "writable", "snapshots", id, "fs")
}
...
func (o *snapshotter) workPathWritable(id string) string {
	return filepath.Join(o.root, "writable", "snapshots", id, "work")
}
